...
const LootCard: React.FC<LootCardProps> = ({ item, width, active = false, isSpinner = false }) => {
  ...
  return (
    ...
    >
      ...
      {!isSpinner && (
        <div 
          className={`
            ...
            w-24 h-24 sm:w-28 sm:h-28 rounded-full blur-[30px] opacity-10 group-hover:opacity-30 transition-opacity duration-500
            ${glowColorClass}
          `}
        ></div>
      )}
      ...
      <div className={`relative z-10 w-24 h-24 sm:w-32 sm:h-32 mb-2 sm:mb-3 flex items-center justify-center ${!isSpinner ? 'transition-transform duration-300 group-hover:scale-110 group-hover:-translate-y-1' : ''}`}>
        {isEmoji ? (
      ...
      <div className="relative z-10 text-center w-full px-1">
        ...
        <h3 className={`font-black italic text-white text-xs sm:text-[13px] leading-tight uppercase tracking-tighter truncate w-full mb-1.5 opacity-90 ${!isSpinner && 'group-hover:opacity-100 transition-opacity'}`}>
          {item.name}
        </h3>
        
        {/* Price tag optimized for spinning visibility */}
        <div className={`inline-flex items-center justify-center bg-[#0d1019]/90 border border-[#2a3040] rounded px-2 py-0.5 sm:px-2.5 shadow-sm ${!isSpinner && 'backdrop-blur-sm group-hover:border-[#FFC800]/50 transition-colors'}`}>
            <span className="text-[#FFC800] font-mono font-bold text-xs sm:text-xs tracking-tighter">
